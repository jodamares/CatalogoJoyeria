...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (materialType !== "ORO" && (!Number.isFinite(fixedBasePrice ?? NaN) || (fixedBasePrice ?? 0) <= 0)) {
      return corsJson(
    ...
    const created = await prisma.product.create({
      data: {
        ...
        categoryId,
        materialType,
        karat: materialType === "ORO" ? karat : null,
        weightGrams: materialType === "ORO" ? weightGrams : 0,
        laborCost: Number.isFinite(laborCost) ? laborCost : 0,
        marginCost: Number.isFinite(marginCost) ? marginCost : 0,
        fixedBasePrice: materialType === "ORO" ? null : fixedBasePrice,
        active,
      },
      include: { category: true },
    });
